...
resource "azurerm_role_assignment" "environment_ci_resource_group" {
  for_each             = { for entry in local.environment_ci_resource_group_roles : "${entry.role}.${entry.resource_group}" => entry }
  scope                = data.azurerm_resource_group.environment_ci_resource_groups[each.value.resource_group].id
