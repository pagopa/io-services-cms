  ...
  ({ blobServiceClient }) =>
    pipe(
      ...
      TE.mapLeft(
        (err) =>
          new H.HttpError(
            `An error occurred retrieving featuredInstitutions file from blobService: [${
...
export const makeFeaturedInstitutionsNewHandler: (
  featuredItemsConfig: FeaturedItemsConfig
) => H.Handler<
  H.HttpRequest,
...
export const GetFeaturedInstitutionsNewFn = (
  featuredItemsConfig: FeaturedItemsConfig
) => httpAzureFunction(makeFeaturedInstitutionsNewHandler(featuredItemsConfig));
