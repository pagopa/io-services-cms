...
export const POST = withJWTAuthHandler(
  async (
    ...
    try {
      ...
      logger.error(
        `An Error has occurred while requesting Ownership Claims for delegate ${params.delegateId}, institution having fiscalCode ${backofficeUser.institution.fiscalCode},
         by selfcareUserId: ${backofficeUser.id}, apimManageSubscriptionId: ${backofficeUser.parameters.subscriptionId}`,
        { error },
      );
...
export const GET = withJWTAuthHandler(
  async (
    ...
    try {
      ...
      logger.error(
        `An Error has occurred while retrieving delegate ${params.delegateId} Ownership claims for institution having fiscalCode ${backofficeUser.institution.fiscalCode},
         requested by selfcareUserId: ${backofficeUser.id}, apimManageSubscriptionId: ${backofficeUser.parameters.subscriptionId}`,
        { error },
      );
