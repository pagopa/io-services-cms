...
export const ApiKeyGroup = ({
  ...
  const isExpandable = apiKey.state === StateEnum.active;

  const canEditCidrs =
    !GROUP_APIKEY_ENABLED || isAdmin(session) || isAdminAggregator(session);

  useEffect(() => {
    if (isExpanded) {
      onExpand(true, subscriptionId);
    }
  ...
  const handleExpandChange = (
    ...
    if (!expanded || isExpandable) {
      onExpand(expanded, subscriptionId);
    }
  };
