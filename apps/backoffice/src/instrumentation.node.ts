...
const applicationInsightConfigurationContent = () =>{
    ...
    const samplingPercentage = process.env["APPLICATIONINSIGHTS_SAMPLING_PERCENTAGE"] as number | undefined;
...
if (process.env["AI_SDK_CONNECTION_STRING"]) {
  ...
  process.env["APPLICATIONINSIGHTS_CONFIGURATION_CONTENT"] = applicationInsightConfigurationContent();
  ...
  .setDistributedTracingMode(
    ai.DistributedTracingModes.AI_AND_W3C
  )
  .start();
