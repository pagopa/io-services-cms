...
import { getInstitutionGroupBaseHandler } from "../get-institution-groups-util";
...
describe("getInstitutionGroupsUtil", () => {
  it("should return a forbidden error response when isInstitutionAllowed check fails", async () => {
    ...
    // when
    const result = await getInstitutionGroupBaseHandler(nextRequest, {
  ...
  `(
    ...
    async ({
      ...
      // when
      await expect(
        getInstitutionGroupBaseHandler(nextRequest, {
          backofficeUser: backofficeUserMock,
          params: { institutionId },
        }),
  ...
  it.each`
    ...
    ${"institutionId"} | ${GroupFilterTypeEnum.ALL}     | ${mocks.retrieveInstitutionGroups}        | ${["institutionId", []]}
  `(
    "should succeed with groups when filter type is $filterType",
    async ({
      institutionId,
      filterType,
      institutionGroupsMock,
