...
export const getLogger = (
  ...
  log: (level: LogLevel, message: string): void => {
    switch (level) {
      case "warn":
        context.warn(`${logPrefix}|${step}|MESSAGE: ${message}`);
        break;
      case "error":
        context.error(`${logPrefix}|${step}|MESSAGE: ${message}`);
        break;
      default:
        context.info(`${logPrefix}|${step}|MESSAGE: ${message}`);
        break;
    }
  },
  ...
  logUnknown: (errs: unknown): void => {
    context.error(`${logPrefix}|${step}|UNKNOWN_ERROR=${JSON.stringify(errs)}`);
  },
});
