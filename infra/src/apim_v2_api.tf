...
resource "azurerm_api_management_api_operation_policy" "get_service_topics_internal_policy_v2" {
  api_name            = "io-services-cms-api"
  api_management_name = data.azurerm_api_management.apim_v2.name
  resource_group_name = data.azurerm_api_management.apim_v2.resource_group_name
  operation_id        = "getServiceTopicsInternal"

  xml_content = file("./api/io_services_cms/v1/getservicetopicsinternal_policy/policy.xml")
}
